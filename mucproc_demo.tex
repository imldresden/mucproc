...
%	anonymous=true, %Für Anonymisierung der Einreichungen
%	forcefinal=true,
	]{mucproc}
...
\title{Formatierungsvorgaben für Beiträge\\
	zur Mensch und Computer 2018}%Tragen Sie hier den Titel Ihrer Einreichung ein.
\author{Vorname Name\thanks[inst]{Abteilung, Institution}\and 
Vorname Name\thanks{Abteilung, Institution} \and Vorname Name\thanksref{inst}}
\email{vorname.name1@institution.de, vorname.name2@..., vorname.name3@...}
	
\maketitle

	
\begin{abstract}
Dieser Text dient dazu die Verwendung der Dokumentenklasse \textsf{mucproc} als Template für Beiträge zur Konferenz \enquote{Mensch und Computer 2018} zu veranschaulichen. Für dieses Dokument wurde die eben genannte Dokumentenklasse verwendet. Es entspricht somit den Formatvorgaben für Einreichungen.
\end{abstract}
...
Bei diesem Dokument handelt es sich um die \LaTeX-Variante der Vorlage für die Mensch und Computer 2018.
Sie verwendet die mit diesem Dokument verteilte Version der \textsf{mucproc}"=Dokumentenklasse \mucprocVersion{} und korrespondiert zur Word-Vorlage für das Jahr 2018.

Sie ist auf aktuellen \TeX-Distributionen kompilierbar, jedoch wird Lua\LaTeX{} für die Einbindung der Open-Type-Schriftart und die korrekte Erstellung eines PDF/A-Formates benötigt.

Um mit \texttt{lualatex} zu kompilieren, können Sie den Vorgang mit der gleichen Syntax, wie bei anderen Compilern starten:
...
\email{name1@institution.de, name2@..., name3@...}
\end{minted}

Bitte verwenden Sie einen kurzen, maximal \textbf{zweizeiligen} Titel und \textbf{keinen} Untertitel.

Die einzelnen Autoren innerhalb des \inlinemint{latex}|\author|-Makros werden durch \inlinemint{latex}|\and| getrennt.
Eine Institution kann über das \texttt{\textbackslash{}thanks}-Makro wie eine Fußnote gesetzt werden, vgl. Beispieldatei. Bitte machen Sie nur Angaben zur Institution und schreiben Sie weitere Informationen (Kurzvita) zu den Autoren an das Ende Ihres Beitrags (siehe \ref{subsec:authors}). Geben Sie für jeden Autor eine E-Mail-Adresse an.
...
Geben Sie innerhalb des \inlinemint{latex}|\email|-Makros bitte die E-Mail-Adressen der Autoren in der entsprechenden Reihenfolge der Nennung an. 
...
\begin{minipage}{\dimexpr.5\linewidth-.5em\relax}
\begin{minted}{latex}
...
Nutzen Sie für die Positionierung von Abbildungen und Tabellen die Standard-Umgebungen \inlinemint{latex}|figure| bzw. \inlinemint{latex}|table|. Verändern Sie, wenn möglich nicht die voreingestellten Positionierungs"-parameter. Setzen Sie Bild- und Tabellenunterschriften.
...
\begin{figure}
\centering
\includegraphics[width=.25\linewidth]{picture}
...
\begin{table}
	\begin{tabularx}{\linewidth}{@{}rX@{}}
		\toprule
		\multicolumn{1}{c}{\itshape Syntax}&\itshape Wirkung\\
		\midrule[\heavyrulewidth]
		\multicolumn{2}{@{}l}{Tabellen mit fester Breite mithilfe von \textsf{tabularx}}\\
		\midrule
		...
		\midrule[\heavyrulewidth]
		\multicolumn{2}{@{}l}{Weitere hilfreiche Befehle bei Tabellen}\\
		\midrule
...
Die Literaturverweise werden, wie auch das Literaturverzeichnis durch das \textsf{biblatex}-Paket mit Biber als Backend erzeugt.

Literaturverweise erscheinen im Text in Klammern, z.\,B. \parencite{Nake.1993} oder bei wörtlichen Zitaten in der Form \parencite[S.~14ff.]{Nake.1993}. Verwenden Sie für die Verweise \inlinemint{latex}|\parencite| mit der folgenden Syntax:
...
\subsection{Autoren}\label{subsec:authors}
